...
def iod_with_angles(epochs: Time, angles: Angle, location: EarthLocation):
    ...
    print('locationshape',location.shape)
    obsvectors = [(location[i] * u.meter /u_length).to_value(u.one) for i in range(len(epochs))]
    # targrts = [(np.array(targrt)[i][:3] * u.meter / u_length).to_value(u.one).tolist() +
    #             (np.array(targrt)[i][3:] * (u.meter/u.second) / u_length * u_time).to_value(u.one).tolist()
    #            for i in range(len(epochs))]
    # print(obsvectors)
    print(np.array(obsvectors).shape)
    ...
    time, r, v, residual = _iod_laplace(times, directions, obsvectors)
    # t.append(clock())
    # print(t)
